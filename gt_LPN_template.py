...
def gen(batch_size=32):
    ...
    while True:
        ...
        for i in range(batch_size):
            image , string = next(img)

            X[i] = np.array(image).transpose(1, 0, 2)
            y[i] = [characters.find(x) for x in string]
...
if(opts.modelname == None):
    ...
    ##use check point to test<------  <-------   <------     <------  <-------   <------checkpoint testing
    #model.load_weights("./checkpoint/checkpoint-02-2.62.hdf5") 
...
checkpoint
'''
filepath = "/home/wan/Desktop/Wan_distribution/checkpoint/checkpoint-{epoch:03d}.hdf5"
...
if (opts.testing == False):
    model.fit_generator(gen(opts.batch_size), steps_per_epoch=opts.steps, epochs=opts.epochs,
            callbacks=[checkpoint, evaluator , EarlyStopping(patience=10)],
